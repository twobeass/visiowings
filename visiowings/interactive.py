...
def interactive_menu():
    ...
    if choice == "1":  # Edit
        output_dir = prompt_path(
            "Export directory (for local files)", default=".", must_exist=False
        )
        ...
        sync_delete = prompt_bool(
            "Sync module deletions (delete in Visio if deleted locally)?", default=False
        )
        ...
        args = InteractiveArgs(command="export", output=output_dir, **common_args)
        print("\n📤 Starting Export...\n")
        cmd_export(args)
        ...
        args = InteractiveArgs(command="import", input=input_dir, force=force, **common_args)
        print("\n📥 Starting Import...\n")
        cmd_import(args)
